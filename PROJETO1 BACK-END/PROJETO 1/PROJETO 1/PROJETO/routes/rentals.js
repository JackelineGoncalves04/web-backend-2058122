...
/* EXERCICIO b */
router.delete('/aluguer/delete',function(req,res,next){
    var id = req.query.id;
...
router.put('/teste/:id', function (req, res, next) {
    var id = req.params.id;
    var customer = req.body.customer;
    var car_model = req.body.car_model;

    connection.query(
      "UPDATE FROM rentals SET customer = ? AND car_model = ? WHERE rentals_id = ? ",
      [customer, car_model, id],
      (err, results, fields) => {
        res.send(results);
      }
    );
